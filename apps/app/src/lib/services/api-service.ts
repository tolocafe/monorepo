...
export const api = {
	...
	orders: {
		...
		create: (orderData: CreateOrder) =>
			privateClient
				...
				.json(),
		createTransaction: (data: { guests_count: number; table_id: string }) =>
			privateClient
				.post<{ transaction_id: number }>('transactions/table', {
					json: data,
				})
				.json(),
	...
	transactions: {
		addProducts: (
			...
			privateClient
				.post<{ success: true }>(`transactions/${transactionId}/products`, {
					json: { products },
				})
				.json(),
		...
		get: (transactionId: string) =>
			privateClient
				.get<DashTransaction>(`transactions/${transactionId}`)
				.json(),
		getActive: () =>
			privateClient
				.get<{ transaction: DashTransaction | null }>('transactions/active')
				.json(),
	},
	wallet: {
